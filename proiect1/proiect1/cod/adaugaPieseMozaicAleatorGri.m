function imgMozaic = adaugaPieseMozaicAleatorGri(params)
%
        ...
        while notDone
            ...
            if isempty(locatiiGoale)
                ...
                for i = 1: numel(locatiiCandidat)
                    ...
                    fm = mean(mean(fereastra(:,:,1)));

                    % Calculez dist euclidiana
                    for k = 1:size(CuloareMediePiese,1)
                        de(k) = sqrt(sum( (CuloareMediePiese(k,:)-fm).^2 )) ;
                    end

                    de2 = sqrt(sum( ((CuloareMediePiese-repmat(fm,[size(CuloareMediePiese,1),1])).^2 ) ,2));
